...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (existingCategory.isDefault)
      return Response.json({ ok: false, error: "Default category cannot be updated" }, { status: 400 });

    const slugExists = await prisma.blogCategory.findFirst({ where: { slug, NOT: { id } } });
    if (slugExists)
      return Response.json({ ok: false, error: `Blog category "${name}" already exists` }, { status: 400 });
    ...
    return Response.json({ ok: true, message: `Blog category "${result.name}" updated successfully` });
  } catch (error) {
    console.log(error);
...
export async function DELETE(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!categoryToDelete) return Response.json({ ok: false, message: "Blog category not found" }, { status: 404 });
    if (categoryToDelete.isDefault)
      return Response.json({ ok: false, messge: "Default category cannot be deleted" }, { status: 400 });
    ...
    if (postCount > 0) {
      ...
      if (!defaultCategory) return Response.json({ ok: false, message: "Default category not found" }, { status: 404 });
      ...
      // revalidatePath("/dashboard/admin/blog-category");
      // revalidatePath("/dashboard/admin/blog/create-blog");
      return Response.json({ ok: true, message: `Blog category "${categoryToDelete.name}" deleted successfully` });
    }
  } catch (error) {
    console.log(error);
    return Response.json({ ok: false, error: "Internal Server Error" }, { status: 500 });
