...
export default function ServiceCard({ service: item }: { service: IService }) {
  ...
  return (
    ...
    >
      <h3 className="text-2xl font-bold h-auto lg:h-16 hyphens-auto">{item.title}</h3>
      <p className="h-auto lg:h-24 xl:h-20 2xl:h-26 text-gray-500 text-base leading-relaxed">
        <span className="font-semibold">{purposeLabel}: </span>
        {item.purpose}
      </p>
      <div className="self-start">
        <h4 className="font-semibold mb-2 text-left">{benefitsLabel}:</h4>
        <ul className="text-base text-left leading-relaxed space-y-2 mb-4 h-auto lg:h-40 xl:h-40 2xl:h-42">
          {item.benefits
            .split("|")
            .slice(0, 3)
            .map((feature, i) => (
