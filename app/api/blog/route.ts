...
export async function POST(req: Request) {
  try {
    ...
    if (!existingCategory) {
      ...
      if (!defaultCategory) return Response.json({ ok: false, message: "No category found" }, { status: 404 });
      categoryId = defaultCategory.id;
    }

    if (await prisma.blog.findFirst({ where: { title } })) {
      return Response.json({ ok: false, message: "Blog title already exists" }, { status: 409 });
    }
    ...
    // revalidatePath("/");
    // revalidatePath("/blog");
    return Response.json({ ok: true, message: "Blog created successfully" });
  } catch (error) {
    console.log(error);
    return Response.json({ ok: false, message: "Internal Server Error" }, { status: 500 });
  }
